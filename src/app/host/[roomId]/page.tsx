...
export default function HostRoomPage({ params }: { params: Promise<{ roomId: string }> }) {
  ...
  // 지목 시 3초 대기 긴장감 연출 ('다음 사람은....' 부유 애니메이션 후 카드 등장 및 폭죽)
  useEffect(() => {
  ...
  }, [isPickingAnimation, targetPostForAnimation, supabase])
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-slate-50/70 text-slate-800 p-6 sm:p-10 lg:p-16 flex flex-col justify-center relative overflow-hidden font-sans">
        ...
        {/* ==================== [지목 긴장감 연출: 3초 대기 부유 애니메이션] (요구사항 4) ==================== */}
        <AnimatePresence>
          {isPickingAnimation && (
            <motion.div
              initial={{ opacity: 0 }}
        ...
        {/* ==================== [발표 진행 중 모달 이탈 시 '발표 이어하기' 플로팅 배너] ==================== */}
        <AnimatePresence>
